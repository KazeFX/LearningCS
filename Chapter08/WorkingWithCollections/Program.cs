// Define an alias for a disctionary with string key and string value.
 ...
 * List<string> cities = new() { "Gothenburg", "Stockholm", "Fukuoka" }; */

/* Alternative syntax that passes an array of string values to AddRange method.
 * List<string> cities = new();
 * cities.AddRange(new[] { "Gothenburg", "Stockholm", "Fukuoka" }); */
...
foreach (string name in new[] { "Vader", "Han Solo", "Obi-Wan", "Luke" })
{
...
coffee.Enqueue("Vader"); // Front of the queue.
coffee.Enqueue("Luke");
coffee.Enqueue("Han Solo");
coffee.Enqueue("Obi-Wan");
coffee.Enqueue("Luke"); // Back of the queue.
